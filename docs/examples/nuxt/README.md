...
Второй claim, `context`, — про контекст, а не про личность. onbo ходит к вам
сервер-к-серверу, без cookie браузера, поэтому всё, что ваши ручки читают из
cookie (активный воркспейс, тенант, язык), уедет в значение по умолчанию, и
действие выполнится не там, где человек его ждал. В примере туда кладётся
значение cookie `active_account` под именем `account_id`; уберите или замените
на то, что нужно вашему API.

В токене лежат **значения**, а не заголовки: как их доставить, решает onbo — в
`config/settings.yaml` (`product.headers: { Cookie: "active_account={account_id}" }`)
или прямо в `config/actions.yaml`, где `{account_id}` подставляется в url, body
и query. Подробнее — в [HOWTO-3, §2.4](../../HOWTO-3-embed-chat.ru.md).
