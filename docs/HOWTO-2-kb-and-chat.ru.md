# How-to 2. База знаний, команды, чат и голос

*[English](HOWTO-2-kb-and-chat.md) · Русский*

Продолжение [HOWTO-1-setup.ru.md](HOWTO-1-setup.ru.md): стенд уже поднят
(`docker compose up -d`), модель подключена. Дальше — наполнить ассистента
содержимым и дать сотрудникам чат с голосом.

Все команды ниже выполняются внутри контейнера:
`docker compose exec app onbo ...`. Каталог проекта смонтирован в контейнер, так
что пути к файлам можно писать обычные, от корня репозитория.

---

## 1. Как устроена база знаний

- **Коллекция** — папка материалов с тегами доступа (`department`, `roles`).
- В коллекции лежат **документы** (длинный текст, режется на фрагменты) и
  **Q&A-пары** (готовый ответ на конкретный вопрос).
- Postgres — основное хранилище, его правят руками. Qdrant — производный
  поисковый индекс, он всегда пересобирается из Postgres (`onbo kb reindex`).
- Доступ фильтруется на уровне поиска: сотрудник видит только материалы своего
  отдела/роли плюс общие. Фильтр строится из профиля в базе, а не из текста
  вопроса — попросить «покажи чужие документы» невозможно.

Пара наследует теги коллекции, если у неё не заданы свои.

## 2. Наполнение: четыре способа

### 2.1. Готовые Q&A из YAML-файла (основной способ)

Файл вида `config/kb.example.yaml` (его же удобно скопировать как заготовку):

```yaml
qa:
  - question: "Как оформить отпуск?"
    answer: "Заявка в HR-портале: Профиль → Отпуска → Новая заявка, за 14 дней."
    collection: hr
  - question: "Как оформить возврат средств?"
    answer: "Возвраты оформляет бухгалтерия: приложите номер заказа и причину."
    collection: accounting
    department: accounting      # кто увидит: отдел
    roles: [accountant]         # и/или роли
    video_url: /media/kb/refund.mp4   # необязательный ролик к ответу
    links:                            # необязательно — куда перейти, см. ниже
      - title: "Возвраты"
        url: "https://app.acme.com/orders/refunds"
```

`links` — такое же поле пары, как `question` или `roles`, а не часть текста
ответа. Адресов в `answer` быть не должно: текст объясняет, что делать, а `links` —
куда перейти. Onbo сам подставит список в конец ответа («Ссылки:») для простых
клиентов и отдаст его веб-каналу структурой `results[].links` — чат-виджет может
нарисовать кнопки (см. [гайд 3](HOWTO-3-embed-chat.ru.md)). Если ссылок нет, поле
просто не пишут.

Импорт (идемпотентный — повторный импорт обновляет пары, не плодит дубли):

```bash
docker compose exec app onbo kb import docs/my_faq.yaml
```

### 2.2. Документы: файл, каталог или сайт

```bash
docker compose exec app onbo kb add-doc docs/handbook.md --collection hr
docker compose exec app onbo kb add-doc docs/                --collection hr
docker compose exec app onbo kb add-doc https://wiki.acme.com/onboarding --collection common
docker compose exec app onbo kb add-doc docs/finance.md --collection accounting \
    --department accounting --roles accountant
```

Документ режется на фрагменты автоматически. Форматы: `.md`, `.txt`, `.rst`, а
также `.pdf` и `.docx` (для них нужны дополнительные пакеты — они входят в
образ). При указании каталога он обходится рекурсивно, лишние типы файлов
пропускаются.

### 2.3. Одна пара из командной строки

```bash
docker compose exec app onbo kb add-qa \
  "Как получить доступ к CRM?" "Заявка в Jira, проект ACCESS, шаблон «CRM»." \
  --collection common
```

### 2.4. Веб-панель

`http://localhost:18000/admin` — добавить/поправить/удалить пару, привязать
видео, запустить переиндексацию. Панель пока одностраничная и **без пароля**:
для десятка пар удобно, на реальном объёме и в открытой сети — нет (см. бэклог
`.claude/PLAN3.md`).

### 2.5. Черновик базы прямо из кода продукта (Claude Code)

Самый быстрый способ получить первую базу знаний — дать Claude Code прочитать
исходники вашего продукта и написать Q&A за вас. В репозитории лежат три скилла
в `.claude/skills/`, они подхватываются автоматически, если Claude Code запущен
**из корня репозитория onbo**:

| Скилл | Что делает |
|---|---|
| `/kb-from-code` | черновик Q&A для базы знаний |
| `/actions-from-code` | черновик `config/actions.yaml` (раздел 4) |
| `/kb-video` | mp4-ролик к паре или к приветствию |

**Сборка базы знаний по шагам:**

1. Открываете Claude Code в каталоге onbo и вызываете скилл, передав путь к коду
   продукта:

   ```
   /kb-from-code ~/projects/acme-crm
   ```

   Что ещё можно задать (иначе будет взято по умолчанию): язык базы знаний (по
   умолчанию русский), коллекция (по умолчанию `common`), дополнительные языки
   для мультиязычного продукта.

2. Скилл идёт по репозиторию в порядке полезности — README и доки, роуты и
   контроллеры, экраны настроек/профиля/онбординга, файлы i18n (главный источник
   точных названий кнопок и пунктов меню), шаблоны — и собирает инвентарь фич.
   Весь репозиторий он не читает.

3. Пишет 15–40 пар в `tmp/kb-from-code/<проект>/draft_faq.yaml` языком новичка,
   а не инженера: без путей API, имён функций и таблиц. Где путь в UI
   восстановить не удалось, остаётся пометка `# TODO: проверить путь в UI`.
   Найденные в коде проверки ролей (`@requires_role("accountant")`,
   `if user.is_staff`) превращаются в `department`/`roles` у пары — материал
   увидят только те, кому положено.

4. В конце скилл показывает сводку: сколько пар, по каким разделам, у каких
   ограничена аудитория, где стоят TODO. **Черновик надо вычитать.** Модель
   додумывает детали, которых в продукте нет, и это самый дешёвый момент их
   поймать.

5. По вашему «да» — импорт:

   ```bash
   onbo kb import tmp/kb-from-code/acme-crm/draft_faq.yaml
   ```

   Если onbo уже запущен, скилл сначала смотрит `GET /admin/api/qa` и не
   предлагает вопросы, которые уже есть. Импорт всё равно идемпотентен по паре
   `collection + question` — повтор обновляет, а не плодит дубли.

6. Открываете `/admin` и правите формулировки, которые читаются криво.

Когда скилл переспросит, а не будет гадать: продукт без UI (одно API — тогда
нельзя писать «Настройки → …»), продукт с нерусским интерфейсом, монорепо (какой
сервис онбордим).

Видео — отдельный шаг: `/kb-video` записывает молчаливый экранный ролик по
одному флоу в UI, озвучивает его и прикрепляет готовый mp4 к паре — см. раздел 3.

### 2.6. Служебное

```bash
docker compose exec app onbo about        # переиндексировать самодокументацию («что я умею»)
docker compose exec app onbo kb status    # сколько пар/документов сейчас в базе
docker compose exec app onbo kb reindex   # пересобрать Qdrant из Postgres
curl -s http://localhost:18000/admin/api/stats   # сколько всего коллекций/документов/пар
```

## 3. Видео-ролики к ответам

Положите mp4 в `media/kb/` (каталог виден и на хосте, и в контейнере) и укажите
в паре `video_url: /media/kb/refund.mp4`. Веб отдаёт файлы на `/media`
самостоятельно. Для Telegram нужен абсолютный адрес — задайте
`MEDIA_BASE_URL=https://onbo.acme.com` в `.env`, он подставится в ссылки.

## 4. Набор команд (действия над продуктом)

Команда = запись в `config/actions.yaml`. Меняете реестр — меняется поведение
ассистента, код трогать не нужно.

Три режима, по уровню риска:

| Режим | Когда | Поведение |
|---|---|---|
| `chat` | низкий риск (язык, тема оформления) | выполняется сразу |
| `confirm` | важное, но обратимое (email, телефон) | сначала карточка «Ок / Отмена» |
| `link` | чувствительное (пароль, платежи) | в чате не выполняется никогда, выдаётся ссылка на страницу продукта |

`sensitive: true` принудительно переводит действие в `link`.

Простое действие без единой строчки кода — блок `api:` вызывается движком:

```yaml
actions:
  set_language:
    description: "Сменить язык интерфейса"
    mode: chat
    params:
      lang: { type: enum, values: [ru, en], required: true }
    api:
      method: POST
      path: "/api/users/{user_id}/language"     # относительно PRODUCT_API_BASE
      body: { language: "{lang}" }
      success_message: "Язык интерфейса переключён на «{lang}»."
```

С подтверждением и собственной валидацией:

```yaml
  change_email:
    description: "Сменить email"
    mode: confirm
    confirm_prompt: "Поменять email на {new_email}?"
    params:
      new_email: { type: email, required: true }
    handler: handlers.actions.change_email      # необязательно: свой Python-обработчик
    api:
      method: POST
      path: "/api/users/{user_id}/email"
      body: { email: "{new_email}" }
      success_message: "Email изменён на {new_email}."
```

Чувствительное — только ссылкой:

```yaml
  change_password:
    description: "Сменить пароль"
    mode: link
    link_url: "https://app.example.com/settings/security"
    sensitive: true
```

**Пайплайн** — несколько действий одной фразой и одним подтверждением:

```yaml
pipelines:
  new_order:
    description: "Оформить заказ: накладные себе и клиенту, отправка клиенту"
    mode: confirm
    confirm_prompt: "Оформить заказ {order_id} и отправить накладные?"
    roles: [accountant]
    params:
      order_id: { type: string, required: true }
    steps:
      - action: create_invoice_internal
        params: { order_id: "{order_id}" }
      - action: create_invoice_client
        params: { order_id: "{order_id}" }
      - action: send_invoice_to_client
        params: { order_id: "{order_id}" }
    on_error: stop        # stop = прервать после первой ошибки, continue = идти дальше
```

Шагом пайплайна может быть только `chat`/`confirm`-действие: чувствительное в
пакет не попадёт.

### Параметры: что ассистент должен собрать в окне чата

Параметр — это не только имя и `required`. Опишите его человеческими словами,
и одно это описание работает в трёх местах: модель понимает, **что** доставать
из фразы; человек получает вопрос, на который можно ответить; в `llm.json`
внешние потребители видят то же самое.

```yaml
  ship_order:
    description: "Отгрузить заказ"
    mode: confirm
    confirm_prompt: "Отгрузить заказ #{order_id} ({delivery}), комментарий: {comment}?"
    params:
      order_id:
        required: true
        description: "номер заказа"             # ← как спросить у человека
      delivery:
        type: enum
        values: [courier, pickup]
        required: true
        description: "как доставить"
      comment:
        description: "комментарий к отгрузке"   # необязательный
```

Диалог из этого получается такой:

> — отгрузи заказ курьером
> — Чтобы «Отгрузить заказ», уточните: номер заказа.
> — 12
> — Отгрузить заказ #12 (courier), комментарий: …? **Ок / Отмена**

Что делает движок сам:

- **Пустое значение = незаполненное.** Модель на «не знаю» отвечает `null`
  ничуть не реже, чем просто пропускает параметр. И то и другое означает одно —
  спросить человека. Без этого правила `null` доезжал до продукта как настоящее
  значение, а в подтверждении печаталось слово `None`.
- **Незавершённое действие запоминается** на 10 минут, и следующее сообщение
  читается как ответ на заданный вопрос: `«12»` дозаполняет `order_id`, а не
  уходит в базу знаний отдельным вопросом.
- **Из анкеты можно выйти.** Если в ответе нет ничего похожего на запрошенное,
  незавершённое действие выбрасывается, а сообщение обрабатывается как обычно —
  человек не заперт в форме, если передумал.
- **Необязательный параметр, которого нет**, печатается в подтверждении как `…`.
- **«Завтра» и «на 25 июля» превращаются в дату.** В каждый запрос виджет кладёт
  местное время браузера с поясом, и модель считает даты от него — не от часов
  сервера, который вполне может быть в UTC и уже на сутки впереди. Год в дате без
  года берётся ближайший будущий. Достаточно написать в описании параметра, в
  каком виде вы хотите дату:
  `description: "когда публиковать, в виде 2026-07-25T11:15"`.

### Значения из справочника продукта

`values:` годится, когда список короткий и не меняется (`ru`/`en`). Склады,
проекты, тарифы, контрагентов так не запишешь: человек говорит «со склада в
Милане», а ручка ждёт `warehouse: 3` — номер строки в вашей же таблице, своей в
каждой установке и меняющейся без правки `actions.yaml`. Такой параметр говорит,
**где** список прочитать:

```yaml
      warehouse:
        required: true
        description: "с какого склада"
        lookup:
          path: "/api/orders/{order_id}/warehouses"   # или абсолютный url:
          items: "data"      # где в ответе лежит список ("" — сам ответ)
          value: "id"        # что уходит в ручку
          label: "name"      # как это называет человек
          match: [code, city]  # чем ещё сверять сказанное
```

Дальше движок сам:

- **совпало одно** — подставляет `id`, а в `{warehouse_label}` кладёт название,
  чтобы в подтверждении было «со склада Milano», а не «со склада 3»;
- **совпало несколько** — спрашивает, какое именно, перечисляя названия;
- **не совпало ничего** — отвечает «такого значения нет. Есть: …» настоящим
  списком, а не выдумкой;
- **параметр вообще не назвали** — вопрос сразу со списком: «Уточните: с какого
  склада — Milano, Milano Nord, Torino». Список живой, устареть не может.

Сверка нестрогая: сначала точное совпадение, потом «начинается с», потом
«содержит», и кириллица сравнивается с латиницей («Милано» находит `Milano`).
Точное попадание не превращается в вопрос из-за более длинного соседа: `Milano`
— это `Milano`, даже если рядом есть `Milano Nord`.

Справочник читается **токеном самого спрашивающего** — тем же, которым
выполняется действие, — так что человеку не покажут строк, которых он не увидел
бы руками. Адрес берётся только из этого файла: из сообщения он не приходит
никогда. Ответ кэшируется на минуту, отдельно по адресу и по тому, чьим токеном
его прочитали. Справочник, который зависит от ещё не известного параметра
(`{order_id}` выше), не читается вовсе: сначала спросят заказ.

Черновик реестра из кода продукта:

```bash
docker compose exec app onbo scan /path/to/your/project    # печатает YAML-черновик
```

или скилл `/actions-from-code <путь>` в Claude Code. Черновик обязательно
вычитывается: генерация касается паролей и персональных данных.

### Чтобы команду находили: `keywords` и `examples`

Список команд больше не вставляется в каждый промпт. Он один раз кладётся в
Qdrant и **ищется на каждое сообщение**: в промпт попадает только десяток команд,
похожих на то, что спросили. Сорок команд в промпте — это деньги за каждое
«привет» и модель, которая выбирает хуже.

Значит, команду нужно **находить**, а описания из трёх слов для этого часто мало.
Два необязательных списка это чинят — они уходят в индекс и никогда в промпт, то
есть на ответе не стоят ничего:

```yaml
  delete_project:
    description: "Удалить проект"
    keywords: [снести, убрать, грохнуть, delete project]
    examples:
      - "удали проект «телефон»"
      - "снеси Арбуз"
```

Пишите то, что люди реально набирают, включая сленг и второй язык вашей команды.
`onbo scan` и `/actions-from-code` заполняют оба списка сами.

Сколько команд одно сообщение может показать — `actions.shortlist_size` (по
умолчанию 12). Если команд у вас меньше, отправляется весь список, как раньше, и
всё это неважно.

После правки `config/actions.yaml`:

```bash
docker compose restart app
```

Рестарт сам замечает, что файл изменился, и переиндексирует каталог (рядом с
индексом хранится отпечаток файла). Проверить или сделать руками — пригодится,
когда на один Qdrant смотрит несколько воркеров:

```bash
docker compose exec app onbo actions status     # актуален / устарел / пуст
docker compose exec app onbo actions reindex
```

`actions.autoindex: false` отдаёт индексацию целиком этим командам. Если индекса
нет или Qdrant недоступен — ничего не ломается: в промпт уходит полный каталог,
ровно как раньше, дольше и дороже, но правильно.

## 5. Приветствие новому сотруднику

При первом сообщении (или по явному вызову) ассистент здоровается в три строки:
кто он, «напишите, что нужно» и «спросите “что ты умеешь”». Настройка в
`config/settings.yaml`:

```yaml
welcome:
  enabled: true
  smooth: false        # true = дать LLM переписать эти три строки
  video:
    accounting: /media/welcome/accounting.mp4   # стартовый ролик отделу или роли
  text_overrides:
    support: "Привет! Начни с раздела «Обработка обращений»."
```

Коротко — намеренно. Раньше приветствие перечисляло все команды и просило модель
переписать эту простыню целиком; отсюда и десять секунд ожидания на первом
контакте. Теперь оно ничего не читает и при `smooth: false` вообще не зовёт
модель.

Полный список переехал в явный вопрос. **«что ты умеешь»** (или «список команд»)
отвечает командами из `config/actions.yaml`, доступными этому человеку,
сгруппированными по тому, что произойдёт: сразу, после подтверждения или ссылкой,
— плюс разделы базы знаний, которые он видит. Список **собирается из того, что
реально есть**, без LLM: он не устаревает и не выдумывает возможностей.

Поэтому если в этом списке упомянуты команды, которых у вас нет, — они лежат в
`config/actions.yaml`. Файл приезжает с демо-примерами; начиная работу со своим
продуктом, очистите его:

```yaml
actions: {}
pipelines: {}
```

```bash
docker compose restart app
# приветствие показывается один раз на человека — снимите отметку, чтобы увидеть снова:
docker compose exec postgres psql -U onbo -d onbo -c "UPDATE app_user SET welcomed_at = NULL;"
```

`text_overrides` полностью заменяет текст приветствия для отдела или роли, а
`features.welcome: false` выключает приветствие целиком.

Проверить:

```bash
curl -s -X POST http://localhost:18000/welcome \
  -H 'Content-Type: application/json' -d '{"user_id":"acc1"}'
```

## 6. Веб-чат с голосом

В репозитории лежит готовая страница чата: текст, микрофон и карточки
подтверждения. Приложение отдаёт статику из `media/`, поэтому страницу
достаточно туда положить — она окажется на том же домене, что и API, и никакой
настройки CORS не потребуется:

```bash
cp docs/examples/chat.html media/
# открыть http://localhost:18000/media/chat.html
```

Что делает страница:

- поле `user_id` (демо-пользователи `acc1`, `sup1`, `admin`) — от него зависит,
  что человек видит и что ему разрешено;
- при открытии запрашивает приветствие;
- текст уходит в `POST /chat`;
- кнопка 🎤 — запись с микрофона, отправка в `POST /voice`, там whisper
  распознаёт речь и дальше работает ровно тот же конвейер, что и для текста;
- действия с `mode: confirm` показываются карточкой с кнопками «Ок / Отмена»,
  ответ уходит в `POST /confirm`.

Микрофон браузер даёт только в защищённом контексте: `localhost` подходит, а
удалённый хост потребует HTTPS.

Голос отключается глобально (`STT_ENABLED=false`) или для конкретного канала
(`channels.web.accept_voice` в `config/settings.yaml`).

## 7. HTTP API — для встраивания в свой интерфейс

```bash
# вопрос или команда
curl -s -X POST http://localhost:18000/chat -H 'Content-Type: application/json' \
  -d '{"user_id":"acc1","text":"смени мой email на new@acme.com","locale":"ru"}'
# → {"text":"Нужно подтверждение:\n• Поменять email на new@acme.com?",
#    "results":[{"status":"needs_confirm","action":"change_email",...}],"welcomed":false}

# подтверждение
curl -s -X POST http://localhost:18000/confirm -H 'Content-Type: application/json' \
  -d '{"user_id":"acc1","action":"change_email","approved":true}'
# → {"status":"done","message":"Email изменён на new@acme.com."}

# голос: multipart, поля user_id / audio / locale
curl -s -X POST http://localhost:18000/voice \
  -F user_id=acc1 -F locale=ru -F audio=@voice.webm
# → {"text":"...", "transcript":"смени язык на английский"}

# приветствие
curl -s -X POST http://localhost:18000/welcome -H 'Content-Type: application/json' \
  -d '{"user_id":"acc1"}'
```

Статусы в `results[].status`: `answer` (ответ из базы), `done` (выполнено),
`needs_confirm` (ждёт «Ок»), `needs_input` (не хватает параметров), `link`
(выдана ссылка), `dry_run` (бэкенд продукта не подключён), `failed`.

Полная схема — `http://localhost:18000/docs` (Swagger).

Встраивая чат в свой фронтенд, помните: `user_id` берётся из тела запроса как
есть. Ставьте перед onbo свой прокси, который проверяет сессию сотрудника и сам
подставляет идентификатор, иначе любой сможет представиться кем угодно.
Отдельной страницы чата на `/` пока нет — есть только эта демо-страница.

## 8. Telegram

```ini
TELEGRAM_ENABLED=true
TELEGRAM_BOT_TOKEN=123456:AA...
```

```bash
docker compose exec -d app onbo serve telegram
```

Голосовые сообщения бот принимает и распознаёт тем же движком; `/start`
присылает приветственную подборку.

## 9. Манифест для внешних ИИ-агентов

`GET /llm.json` (и `/.well-known/llm.json`) отдаёт публичную часть базы и список
команд в машинночитаемом виде — чтобы сторонний ассистент, зашедший на ваш сайт,
понимал, что умеет продукт. Выгрузка файлом для статического хостинга:

```bash
docker compose exec app onbo llm-export --out llm.json
```

Приватные материалы (с `department`/`roles`) в манифест не попадают.

## 10. Порядок действий для нового проекта

1. Поднять стенд и подключить модель — файл 1.
2. `PRODUCT_API_BASE` и профили сотрудников (`app_user`) — файл 1, разделы 6–7.
3. Собрать 20–30 Q&A по реальным вопросам новичков (`/kb-from-code` → ревью →
   `onbo kb import`).
4. Описать 3–5 команд в `config/actions.yaml`, начиная с безобидных (`chat`),
   чувствительные — сразу `link`.
5. Проверить на демо-бэкенде (`dry_run`/`done` в ответах), потом переключить на
   реальный API.
6. Дать людям `chat.html` (или встроить `/chat` в свой интерфейс), закрыв
   `/admin` от внешнего доступа.
