...
```yaml
  create_post:
    ...
    params:
      project_id:
        required: true
        description: "в каком проекте"          # ← как спросить у человека
      platform:
        type: enum
      ...
      topic_title:
        description: "заголовок поста"          # необязательный
```

Диалог из этого получается такой:

> — создай пост про арбузы
> — Чтобы «Создать пост», уточните: в каком проекте; площадка (instagram, telegram).
> — в 12-м, инстаграм
> — Создать пост «про арбузы» в проекте #12? **Ок / Отмена**
...
- **Незавершённое действие запоминается** на 10 минут, и следующее сообщение
  читается как ответ на заданный вопрос: `«12»` дозаполняет `project_id`, а не
...
`values:` годится, когда список короткий и не меняется (`ru`/`en`). Площадки,
проекты, склады, тарифы так не запишешь: человек говорит «инстаграм», а ручка
ждёт `platform: 3` — номер строки в вашей же таблице, своей в каждой установке и
меняющейся без правки `actions.yaml`. Такой параметр говорит, **где** список
прочитать:
...
```yaml
      platform:
        ...
        description: "площадка"
        lookup:
          path: "/api/projects/{project_id}/platforms"   # или абсолютный url:
          items: "data"      # где в ответе лежит список ("" — сам ответ)
          value: "id"        # что уходит в ручку
          label: "name"      # как это называет человек
          match: [code]      # чем ещё сверять сказанное
```

Дальше движок сам:

- **совпало одно** — подставляет `id`, а в `{platform_label}` кладёт название,
  чтобы в подтверждении было «на площадке Instagram», а не «на площадке 3»;
...
- **параметр вообще не назвали** — вопрос сразу со списком: «Уточните: площадка
  — Instagram, Telegram, Threads». Список живой, устареть не может.

Сверка нестрогая: сначала точное совпадение, потом «начинается с», потом
«содержит», и кириллица сравнивается с латиницей («телеграм» находит
`Telegram`). Точное попадание не превращается в вопрос из-за более длинного
соседа: `vk` — это `vk`, даже если рядом есть `vk-stories`.
...
(`{project_id}` выше), не читается вовсе: сначала спросят проект.
