...
У действий (и пайплайнов) есть опциональные `department` и `roles` — семантика та же, что у базы знаний: пусто = доступно всем, иначе действие предлагается только пользователям с подходящим профилем. Это влияет и на роутинг, и на список «что ты умеешь».

### Как находится нужная команда

Каталог не вставляется в каждый промпт. Он лежит в том же Qdrant, что и база знаний (отдельный `kind`, поэтому одно никогда не всплывает в выдаче другого), и **ищется на каждое сообщение**: в модель уходят только команды, похожие на спрошенное, отфильтрованные на стороне сервера тем же правилом «отдел/роли».

Поэтому команду нужно находить, а короткого `description` для этого часто мало. Два необязательных списка уходят в индекс и никогда в промпт:
...
Настройки: `actions.shortlist_size` (по умолчанию 12 — если команд меньше, отправляется весь каталог) и `actions.autoindex` (переиндексация на старте, когда `actions.yaml` изменился; иначе `onbo actions reindex`). Индекс пуст или Qdrant недоступен — в промпт уходит полный каталог, как раньше: дольше и дороже, но правильно.
...
При первом контакте ассистент здоровается в три строки: кто он, приглашение написать, что нужно, и подсказка спросить «что ты умеешь». Коротко и офлайн намеренно: ничего не читается и модель не зовётся — приветствие это то самое сообщение, где ожидание заметнее всего. `welcome.smooth: true` разрешает LLM переписать эти три строки; опционально прикрепляется стартовый ролик на роль/отдел (`welcome.video`), а `welcome.text_overrides` заменяет текст целиком.

Сам список — на один вопрос дальше: **«что ты умеешь» / «список команд»** отвечает действиями и пайплайнами, доступными именно этому человеку, сгруппированными по режиму, плюс разделы базы знаний, о которых он может спросить. Фильтр доступа тот же, что у КБ, LLM в этом пути нет вовсе. Триггеры приветствия: `POST /welcome` (веб), `/start` (Telegram) и автоматически на первом сообщении нового пользователя — оно идёт перед его настоящим ответом, а не вместо вопроса. Управляется `welcome.enabled` и флагом `features.welcome`.
...
onbo kb import ./faq.yaml                   # массовый импорт Q&A-пар (образец — config/kb.example.yaml)
onbo kb reindex                             # перестроить индекс из Postgres
onbo kb status                              # что сейчас лежит в базе знаний
onbo about                                  # проиндексировать доки о себе
onbo actions reindex                        # проиндексировать config/actions.yaml для поиска команд
onbo actions status                         # индекс ещё соответствует файлу?
onbo users add u42 --department accounting --roles accountant
onbo users import ./users.yaml              # массовый импорт справочника (образец — config/users.example.yaml)
...
```bash
cp config/kb.example.yaml faq.yaml    # и заменить примеры своими ответами
./run.sh kb import ./faq.yaml
./run.sh kb add-doc ./handbook.md --collection common   # файл, папка или URL
./run.sh kb status                                      # проверить, что легло
```

Кто что видит — задаёт справочник пользователей, он тоже стартует пустым: всем, кого
там нет, достаётся минимум прав (`roles: [employee]`, без отдела, только публичный
контент):
...
Первые 12 пунктов реализованы и доведены (действия над профилем по реальному HTTP с демо-бэкендом, визуальная админка и набор тестов pytest). Вторая волна (13–19) добавляет скиллы Claude Code, пайплайны действий, машиночитаемый манифест, фичефлаги и проактивный welcome — готово.
