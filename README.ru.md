...
Рекомендуемый локальный вариант: **инфраструктура в Docker**, а **приложение на хосте** — так faster-whisper (STT) и локальная LLM используют вашу GPU напрямую. По умолчанию LLM — локальная модель через [Ollama](https://ollama.com), облачный ключ не нужен.

```bash
docker compose up -d          # только инфра: qdrant + postgres + redis
ollama pull llama3.2:3b       # локальная модель-классификатор по умолчанию
...
которые едут внутри venv, — так faster-whisper грузится на GPU. Чтобы вместо Ollama использовать
облачную LLM, задайте `LLM_MODEL` / `LLM_API_KEY` в `.env` (см. `.env.example`).
